(function () {
// API CONFIGURATION.
// Replace these endpoints with your actual API URLs.
const API_CONFIG = {
    videosUrl: 'https://ada.skynettechnologies.us/api/video-subtitle/details',
    planPackagesUrl: 'https://ada.skynettechnologies.us/api/video-subtitle/packages',    
    activeLanguages: 'https://ada.skynettechnologies.us/api/active-language',
    apiURL: 'https://ada.skynettechnologies.us/api/',    
    headers: {
...
async function fetchVideos(websiteUrl) {
    const normalizedWebsiteUrl = window.location.origin;
...
function renderHeader(isPurchased, plan) {
   const container = $id('headerRight');
    if (!container) {
        return;
    }
    if (isPurchased && plan) {
        ...
        `;
           const activeStatus = container.querySelector('.badge-status');
                    if (activeStatus) {
                ...
                activeStatus.addEventListener('dblclick', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                });
            }
        $id('upgradeBtn')?.addEventListener(
            'click',
            openUpgradeModal,
        );
        return;
...
function renderPlanCards(plans, isYearly = false) {
    const container = $id('planCards');
    if (!container) {
        return;
    }
                    ...
                    <div class="card plan-card h-100">
                        ...
                        <div class="card-body">
                            <ul class="plan-features">
                                <li>
                                    Unlimited number of videos, with total
                                    playback time of up to
                            ...
                            <button
                                type="button"
                                class="btn btn-outline-primary plan-btn"
...
$id('statVideos').textContent = analytics.totalVideos;
$id('statTotalLimit').innerHTML = `${totalMin} <small style="font-size:14px;font-weight:400;">min</small>`;
$id('statSpent').textContent = analytics.totalDurationFormatted;
$id('statRemaining').textContent = formatMinSec(analytics.remainingSeconds);
const badge = $id('lowRemainingBadge');
...
function renderCircularProgress(percentage, totalSec, totalMin) {
    const container = $id('circularContainer');
    const size = 180;
    const stroke = 20;
...
function renderVideoGrid(videos) {
    const grid = $id('videoGrid');
    const empty = $id('emptyState');
    const badge = $id('videoCountBadge');
...
function openVideoModal(embedUrl) {
    ...
    iframe.src = embedUrl;
    const modal = new bootstrap.Modal($id('videoModal'));
    modal.show();
    $id('videoModal').addEventListener('hidden.bs.modal', function() {
        iframe.src = '';
    }, { once: true });
}
...
function buildDateDropdown() {
    const menu = $id('dateDropdownMenu');
    const label = $id('dateRangeLabel');
    ...
    if (!isCustom) {
        menu.querySelectorAll('[data-range]').forEach(btn => {
            btn.addEventListener('click', function(e) {
                ...
                bootstrap.Dropdown.getInstance($id('dateDropdown')).hide();
            });
        });
...
async function enableLanguageToggle(){
    ...
    const checkbox = $id('lang-widget-toggle');
...
function updateAnalytics() {
    ...
    // 6. Quota alert
    const quotaAlert = $id('quotaAlert');
    ...
    // 8. Video count
    $id('videoCountBadge').textContent = `${filtered.length} Videos`;
}
...
function openUpgradeModal() {
    const body = $id('upgradeModalBody');
    const modalElement = $id('upgradeModal');
    ...
    if (!upgradePlans.length) {
        body.innerHTML = `
           <div class="alert alert-warning mb-0">
                To upgrade your plan please contact us at <br> 'hello@skynettechnologies.com'
            </div>
        `;
        new bootstrap.Modal(modalElement).show();
        return;
    ...
    const selectBtn = $id('upgradeSelectBtn');
...
// TOGGLE BETWEEN PLAN SELECTION & DASHBOARD
function renderApp() {
    const planSelection = $id('planSelection');
    const dashboard = $id('analyticsDashboard');
    if (!planSelection || !dashboard) {
        return;
    }
    if (isPurchased && currentPlan) {
        planSelection.style.display = 'none';
        dashboard.style.display = 'block';
        renderHeader(true, currentPlan);
        updateAnalytics();
        return;
    }
    ...
    renderPlanCards(PLAN_PACKAGES, false);
}
...
document.addEventListener(
    ...
    async function () {
        ...
        enableLanguageToggle();
        buildDateDropdown();
        $id('lang-widget-toggle')?.addEventListener('change', function () {
            const isChecked = this.checked;
            console.log(isChecked); // true or false
            updateLanguageToggle(isChecked);
        });
        $id('languageFilter')?.addEventListener(
            'change',
            function () {
        ...
        renderApp();
        $id('dateDropdown')?.addEventListener(
            'show.bs.dropdown',
            function () {
                buildDateDropdown();
            },
